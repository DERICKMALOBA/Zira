...
const CustomerVerification = ({ customerId, onClose }) => {
 ...
 useEffect(() => {
  ...
  const fetchCustomerDetails = async () => {
    try {
      setLoading(true);
       console.log("🚀keep it up fetchCustomerDetails started with customerId:", customerId);
       console.log("👀 Strict mode double render check", new Date().toISOString());
...
if (userRole === "credit_analyst_officer") {
  ...
  console.log(" Fetching CA-specific verification data for customer:", customerId);
  ...
  if (!csoRow) {
    console.warn(" No CSO verification data found for this customer (this is normal if CSO hasn't reviewed yet)");
  }
} else {
  ...
  const handleVerificationChange = (field, value, section = "customer", index = null) => {
    setVerificationData((prev) => {
      ...
      } else if (["security", "guarantorSecurity", "loan", "business", "nextOfKin", "document"].includes(section)) {
  ...
  const submitVerification = async () => {
    try {
      ...
      const { error } = await supabase.from("customer_verifications").insert(baseData);
      if (error) throw error;

      // Determine new status based on role and decision
      let newStatus;
      if (userRole === 'branch_manager') {
        if (verificationData.finalDecision === "approved" || verificationData.finalDecision === "referred") {
          newStatus = "cso_review";
        } else if (verificationData.finalDecision === "pending" || verificationData.finalDecision === "edit") {
  ...
  // // Role-specific Loan Step Component
  // const LoanStep = () => {
  //   if (userRole === 'credit_analyst_officer') {
  //     return (
  //       <div className="p-8">
  ...
  //                   <span className="text-blue-600 font-bold text-xs">INIT</span>
  //                 </div>
  //               </div>
  //               <p className="text-2xl font-bold text-blue-700 mb-2">
  //                 KES {customer?.prequalifiedAmount?.toLocaleString("en-US") || "0"}
  //               </p>
  //               <p className="text-sm text-blue-600">Initial system assessment</p>
  //             </div>
  ...
  //                   <span className="text-purple-600 font-bold text-xs">BM</span>
  //                 </div>
  //               </div>
  //               <p className="text-2xl font-bold text-purple-700 mb-2">
  //                 KES {bmScoredAmount?.toLocaleString("en-US") || "0"}
  //               </p>
  //               <p className="text-sm text-purple-600">Branch Manager assessment</p>
  //             </div>

  //             {/* CSO Comment */}
  //             <div className="bg-gradient-to-br from-amber-50 to-yellow-50 p-6 rounded-xl border border-amber-100">
  //               <div className="flex items-center justify-between mb-4">
  //                 <h4 className="font-semibold text-amber-900">CSO Comment</h4>
  ...
  //                 {csoComment ? (
  //                   <p className="text-sm text-gray-700">{csoComment}</p>
  //                 ) : (
  //                   <p className="text-xs text-gray-500 italic">No comment yet</p>
  //                 )}
  //               </div>
  //               <p className="text-sm text-amber-600 mt-2">CSO feedback</p>
  //             </div>

  //             {/* CSO Final Decision */}
  //             <div className="bg-gradient-to-br from-orange-50 to-red-50 p-6 rounded-xl border border-orange-100">
  //               <div className="flex items-center justify-between mb-4">
  //                 <h4 className="font-semibold text-orange-900">CSO Decision</h4>
  ...
  //                 {csoDecision ? (
  //                   <p className="text-lg font-bold text-orange-700 capitalize">{csoDecision}</p>
  //                 ) : (
  //                   <p className="text-xs text-gray-500 italic">No decision yet</p>
  //                 )}
  //               </div>
  //               <p className="text-sm text-orange-600 mt-2">Final CSO decision</p>
  //             </div>
  //           </div>
  //         </div>
  ...
  //               <div className="flex items-center justify-between mb-4">
  //                 <h4 className="font-semibold text-emerald-900">Your Scored Amount</h4>
  //                 <div className="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center">
  //                   <span className="text-emerald-600 font-bold text-xs">CA</span>
  //                 </div>
  //               </div>
  //               <div className="flex items-center mb-2">
  //                 <span className="text-xl font-bold text-emerald-700 mr-2">KES</span>
  //                 <input
  //                   type="number"
  ...
  //                 />
  //               </div>
  //               <p className="text-sm text-emerald-600">Enter your assessment amount</p>
  //             </div>

  //             {/* Visual Comparison */}
  //             <div className="bg-gradient-to-br from-gray-50 to-slate-50 p-6 rounded-xl border border-gray-200">
  //               <h4 className="font-semibold text-gray-900 mb-4">Amount Comparison</h4>
  //               <div className="space-y-3">
  //                 <div className="flex justify-between items-center">
  //                   <span className="text-sm text-gray-600">Prequalified:</span>
  //                   <span className="text-sm font-bold text-blue-700">
  //                     KES {customer?.prequalifiedAmount?.toLocaleString("en-US") || "0"}
  //                   </span>
  //                 </div>
  //                 <div className="flex justify-between items-center">
  //                   <span className="text-sm text-gray-600">BM Scored:</span>
  ...
  //                     KES {verificationData.loan.scoredAmount?.toLocaleString("en-US") || "0"}
  //                   </span>
  //                 </div>
  //               </div>
  //             </div>
  ...
  const ToggleSwitch = ({ checked, onChange, label }) => (
    <label className="flex items-center cursor-pointer group">
      ...
      <div className={`relative w-14 h-7 bg-gray-300 rounded-full transition-colors duration-200 ${
        checked ? "bg-gradient-to-r from-emerald-400 to-emerald-500" : "hover:bg-gray-400"
      }`}>
        <div className={`absolute top-0.5 left-0.5 bg-white border rounded-full w-6 h-6 transition-transform duration-200 shadow-md ${
          checked ? "transform translate-x-7 shadow-emerald-200" : "shadow-gray-300"
        }`}>
          {checked && <CheckCircleIcon className="h-4 w-4 text-emerald-500 m-0.5" />}
        </div>
      </div>
      <span className={`ml-3 text-sm font-medium transition-colors ${
        checked ? "text-emerald-700" : "text-gray-700 group-hover:text-gray-900"
      }`}>
  ...
  const DocumentCard = ({ title, imageUrl, placeholder, icon: Icon }) => (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-200">
      ...
      <div className="p-4">
        {imageUrl ? (
          <div className="relative group cursor-pointer" onClick={() => setSelectedImage({ url: imageUrl, title })}>
            <img src={imageUrl} alt={title} className="w-full h-48 object-contain rounded-lg bg-gray-50 border border-gray-100 group-hover:scale-105 transition-transform duration-200" />
  ...
  const validateCurrentStep = () => {
    switch (step) {
      ...
      case 3: // Loan Assessment
        ...
        break;
      case 4: // Guarantors Verification
  ...
  const steps = [
    ...
    { num: 3, label: "Loan", icon: CurrencyDollarIcon },
    { num: 4, label: "Guarantors", icon: UserGroupIcon },
    { num: 5, label: "Security", icon: ShieldCheckIcon },
    { num: 6, label: "Next of Kin", icon: UserCircleIcon },
    { num: 7, label: "Documents", icon: DocumentTextIcon },
            ...
            <div>
              ...
              <div className="mt-2 flex items-center">
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                  ...
                  {userRole === 'branch_manager' ? 'Branch Manager' : 'Credit Analyst'} Mode
                </span>
              </div>
            </div>
            <button
