...
const AllLoans = () => {
  ...
  const isRegionalLevel = ["regional_manager", "credit_analyst_officer", "customer_service_officer"].includes(profile?.role);
  ...
  const fetchBranches = async () => {
    try {
      ...
      // Regional level: only branches in their region
      else if (isRegionalLevel && profile?.region_id) {
  ...
  const fetchLoans = async () => {
    try {
      ...
      // Regional level: only loans from their region
      else if (isRegionalLevel && profile?.region_id) {
        // Filter by region through the customers.branches.region_id relationship
  ...
  const filterLoans = () => {
    ...
    // Only allow region filter for super admin and regional level users
    if (regionFilter !== "all" && !isBranchManager) {
      filtered = filtered.filter(
        (loan) => loan.customers?.branches?.region_id === regionFilter
      );
    }

    // Only allow branch filter for super admin and regional level users
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-indigo-100">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            {/* Region Filter - Only for super admin and regional level */}
            {!isBranchManager && (
              <select
                value={regionFilter}
                onChange={(e) => setRegionFilter(e.target.value)}
                className="p-3 border border-gray-300 rounded-lg"
                disabled={isRegionalLevel}
              >
                <option value="all">
                  {isRegionalLevel ? profile?.region || "All Regions" : "All Regions"}
                </option>
                {isSuperAdmin && regions.map((region) => (
            ...
            {/* Branch Filter - Only for super admin and regional level */}
            {!isBranchManager && (
            ...
            <tbody className="divide-y divide-gray-200 text-sm">
              {filteredLoans.map((loan, index) => (
                ...
                >
                  ...
                  <td className="px-3 py-3 whitespace-nowrap">{loan.customers?.branches?.name}</td>
