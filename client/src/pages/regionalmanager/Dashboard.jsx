...
if (regionError) {
 ...
 console.log("region name 2", userRegion)
      ...
      if (activeCustomersError) console.error('Active customers error:', activeCustomersError);
      ...
      if (approvalsError) console.error('Pending approvals error:', approvalsError);
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-8 py-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-1">Regional Branches</h3>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Branch Details</th>
              ...
              <tr key={branch.id} className="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    branch.status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'
                  }`}>
  ...
  const renderCustomersTable = () => (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      <div className="bg-gradient-to-r from-emerald-50 to-teal-50 px-8 py-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-1">Regional Customers</h3>
            <p className="text-sm text-gray-600">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {customers.map((customer) => (
              <tr key={customer.id} className="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    customer.verification_status === 'verified' ? 'bg-emerald-100 text-emerald-800' : 
                    customer.verification_status === 'pending' ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'
                  }`}>
                    <div className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                      customer.verification_status === 'verified' ? 'bg-emerald-500' : 
                      customer.verification_status === 'pending' ? 'bg-amber-500' : 'bg-red-500'
                    }`}></div>
  ...
  const renderLoansTable = () => (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 px-8 py-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-1">Regional Loans</h3>
            <p className="text-sm text-gray-600">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loans.map((loan) => (
              <tr key={loan.id} className="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    loan.status === 'booked' ? 'bg-emerald-100 text-emerald-800' : 
                    loan.status === 'pending' ? 'bg-amber-100 text-amber-800' : 
                    loan.status === 'defaulted' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                  }`}>
                    <div className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                      loan.status === 'booked' ? 'bg-emerald-500' : 
                      loan.status === 'pending' ? 'bg-amber-500' : 
                      loan.status === 'defaulted' ? 'bg-red-500' : 'bg-gray-500'
                    }`}></div>
                    {loan.status || 'unknown'}
                  </span>
                </td>
                <td className="px-6 py-4 text-sm text-gray-500">{getBranchName(loan.branch_id)}</td>
                <td className="px-6 py-4 text-sm text-gray-500">
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50">
        <div className="text-center">
          <div className="relative">
            ...
            <div className="animate-pulse absolute inset-0 rounded-full h-16 w-16 border-4 border-transparent border-t-purple-400 mx-auto" style={{animationDelay: '0.5s'}}></div>
          </div>
          <p className="mt-6 text-lg text-gray-600 font-medium">Loading regional dashboard...</p>
                  ...
                  <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className={`flex items-center text-sm font-semibold ${
                        ...
                        <svg className={`h-4 w-4 mr-1 ${stat.trend === 'up' ? 'rotate-0' : 'rotate-180'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {loading ? (
                    ...
                    <div className="space-y-4">
                      {recentActivity.map((activity, index) => (
                        <div key={activity.id} className="flex items-center p-4 bg-gradient-to-r from-gray-50 to-white rounded-xl hover:shadow-md transition-all duration-200">
                          <div className={`rounded-xl p-3 mr-4 ${activity.iconBg} shadow-sm`}>
                            {activity.icon}
                          </div>
                          <div className="flex-1">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <p className="text-sm font-bold text-gray-700">{activity.amount}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                      {recentActivity.length === 0 && (
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-500 font-medium">No recent activity in your region</p>
                        </div>
                      )}
                    </div>
                  )}
                ...
                <div className="space-y-6">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-sm font-semibold text-gray-600">Portfolio Growth</p>
                      <span className="text-sm font-bold text-emerald-600">+{performanceMetrics.portfolioGrowth}%</span>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-sm font-semibold text-gray-600">Approval Rate</p>
                      <span className="text-sm font-bold text-blue-600">{performanceMetrics.approvalRate}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                      <div 
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-sm font-semibold text-gray-600">Default Rate</p>
                      <span className="text-sm font-bold text-red-600">{performanceMetrics.defaultRate}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                      <div 
